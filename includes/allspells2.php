...
function allspellsinfo2(&$row, $level=0)
{
	...
	if(($row['effect1itemtype']) and (!($row['effect1Aura'])))
	{
		if(IsSet($allitems[$row['effect1itemtype']]['icon']))
			...
			$allspells[$num]['icon'] = $DB->selectCell('SELECT iconname FROM ?_icons WHERE id=(SELECT displayid FROM ?_item_template WHERE entry=?d LIMIT 1) LIMIT 1',$row['effect1itemtype']);
	} else {
