...
function transform_point($at, $point)
{
	...
	if (isset($point['spawntimesecs']))
	{
		...
		if (isset($point['spawnwindowsecs']) && $point['spawnwindowsecs'])
			$result['mr'] = sec_to_time($point['spawntimesecs'] + $point['spawnwindowsecs']);
	}
	unset($result['spawntimesecs']);
	unset($result['spawnwindowsecs']);
...
function position($id, $type, $spawnMask = 0)
{
	...
	$data = $DB->select('
			SELECT guid, map AS m, position_x AS x, position_y AS y, spawntimesecs, {spawnwindowsecs, MovementType AS ?#, }"0" AS `type`
			FROM '.$type.'
			WHERE id = ?d
